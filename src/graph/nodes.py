    ...
    if should_validate:
        ...
        if configurable.enforce_researcher_search:
            ...
            if not web_search_validated:
                logger.warning(
                    f"[VALIDATION] Researcher did not use web_search tool. Adding reminder to response."
                )
...
async def researcher_node(
    ...
    logger.info(
        f"[researcher_node] enforce_researcher_search is set to: {configurable.enforce_researcher_search}"
    )
